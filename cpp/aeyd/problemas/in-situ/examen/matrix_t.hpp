...
{
  assert(A.get_n() == B.get_m()); // n columnas m filas
  resize(A.get_m(), B.get_n());
  for (int i = 1; i <= A.get_m(); ++i) {
    for(int j = 1; j <= B.get_n(); ++j) {
      at(i, j) = 0;
      for (int k = 1; k <= A.get_n(); ++k) { // k para recorrer filas
...
template<class T>
T matrix_t<T>::Sarrus() {
  T det = 0, mult = 0, first = 1, second = 1;
  assert(m_ == 3 && n_ == 3);
  for (int i = 0; i < m_; ++i) {
    first = 1;
    second = 1;
    for (int j = 0; j < n_; ++j) {
      first *= at(1 + j, 1 + (j + i) % 3);
      second *= at(3 - j, 1 + (j + i) % 3);
    }
    det += first - second;
  }
  return det;
